...
#find first year and filter ----------------------------------------------------------------------
first_year = 2021  

data_first_year <- data_trimmed %>%
  filter(YEAR == first_year)

#compare owners vs rent in 2021---------------------------------------------------------------
rentown_table  <- table(data_first_year$OWNERSHP, useNA = "ifany")
rentown_propTable <- prop.table(rentown_table)
round(rentown_propTable, 3)

#Observations-----------------------------------------------------------
first_year
nrow(data_first_year)
table(data_first_year$YEAR)

#Distribution table for 2021----------------------------------------------------------------------
own_dist <- as.data.frame(rentown_table) %>%
  rename(OWNERSHP = Var1, n = Freq) %>%
  mutate(prop = n / sum(n))
own_dist

#Summaries of age, income, and rent in 2021---------------------------------------
summary(data_first_year$AGE)
summary(data_first_year$HHINCOME)
summary(data_first_year$RENTGRS)

#Clean income and rent variables---------------------------------------
data_first_year_clean <- data_first_year %>%
  mutate(
...
data_first_year_burden <- data_first_year_clean %>%
  mutate(
    #Simple tenure classification
    ...
    #1. Rent used for burden calculation
    ...
    rent_for_burden = case_when(
      ...
      #2. If OWNERSHPD is just Rented, use RENTGRS_clean  
...
summary(data_first_year_burden$rent_burden)
  

#Categories for renters------------------------------------------------------------------
renters_burden_cat <- data_first_year_burden %>%
  filter(
    tenure_simple == "Renter",
    !is.na(rent_burden),
    rent_burden >= 0,
    rent_burden < 3      # trim crazy outliers (300%+)
  ) %>%
  mutate(
...
owners_burden_cat <- data_first_year_burden %>%
  filter(
    tenure_simple == "Owner",
    !is.na(owner_burden),
    owner_burden >= 0,
    owner_burden < 3      # trim extemely high outliers (300%+)
  ) %>%
  mutate(
...
ggplot(
  ...
  labs(
    title = paste("Rent Burden Distribution (Young Adult Renters -", first_year, ")"),
...
ggplot(
  ...
  labs(
    title = paste("Owner Mortgage Burden Distribution (Young Adult Owners with/without mortgage -", first_year, ")"),

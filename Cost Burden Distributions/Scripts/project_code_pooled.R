...
# Clean income and rent for ALL years----------------------------------------------
data_all_clean <- data_trimmed %>%
  mutate(
...
# Add tenure and renter & owner burdens for ALL years----------------------------------------
data_all_burden <- data_all_clean %>%
  mutate(
    ...
    #3. Owner mortgage cost used for owner burden  
...
renters_burden_pooled <- data_all_burden %>%
  filter(
    tenure_simple == "Renter",
    !is.na(rent_burden),
    rent_burden >= 0,
...
renters_burden_pooled

#--------------------------------------------------------------------------------------

owners_burden_pooled <- data_all_burden %>%
  filter(
    tenure_simple == "Owner",
    !is.na(owner_burden),
    owner_burden >= 0,
...
ggplot(
  ...
  labs(
    title = "Pooled Renter Burden Distribution (All Years, Young Adult Renters)",
...
ggplot(
  ...
  labs(
    title = "Pooled Owner Mortgage Burden (All Years, Young Adult Owners)",
    x = "Owner Burden (Mortgage / Income, %)",
    y = "Number of Owners"
  )

...
#find third year and filter ----------------------------------------------------------------------
third_year = 2023

data_third_year <- data_trimmed %>%
  filter(YEAR == third_year)

#compare owners vs rent in 2023---------------------------------------------------------------
rentown_table  <- table(data_third_year$OWNERSHP, useNA = "ifany")
rentown_propTable <- prop.table(rentown_table)
round(rentown_propTable, 3)

#Observations-----------------------------------------------------------
third_year
nrow(data_third_year)
table(data_third_year$YEAR)

#Distribution table for 2023----------------------------------------------------------------------
own_dist <- as.data.frame(rentown_table) %>%
  rename(OWNERSHP = Var1, n = Freq) %>%
  mutate(prop = n / sum(n))
own_dist

#Summaries of age, income, and rent in 2023---------------------------------------
summary(data_third_year$AGE)
summary(data_third_year$HHINCOME)
summary(data_third_year$RENTGRS)

#Clean income and rent variables---------------------------------------
data_third_year_clean <- data_third_year %>%
  mutate(
...
data_third_year_burden <- data_third_year_clean %>%
  mutate(
...
summary(data_third_year_burden$rent_burden)

#Categories for renters------------------------------------------------------------------

renters_burden_cat <- data_third_year_burden %>%
  filter(
    tenure_simple == "Renter",
    !is.na(rent_burden),
    rent_burden >= 0,
...
owners_burden_cat <- data_third_year_burden %>%
  filter(
    tenure_simple == "Owner",
    !is.na(owner_burden),
    owner_burden >= 0,
...
ggplot(
  ...
  labs(
    title = paste("Rent Burden Distribution (Young Adult Renters -", third_year, ")"),
...
ggplot(
  ...
  labs(
    title = paste("Owner Mortgage Burden Distribution (Young Adult Owners With/Without Mortgage -", third_year, ")"),
